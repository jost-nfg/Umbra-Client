...
import net.umbra.gui.components.widgets.UmbraButtonWidget;
import net.umbra.gui.components.widgets.UmbraImageButtonWidget;
import net.umbra.gui.screens.addons.AddonScreen;
...
public class MainMenuScreen extends Screen {
	...
	public MainMenuScreen() {
		super(Component.nullToEmpty("Umbra Client Main Menu"));
	}
	...
	public void init() {
		...
		UmbraButtonWidget singleplayerButton = new UmbraButtonWidget(startX, startY, BUTTON_WIDTH, BUTTON_HEIGHT,
		...
		UmbraButtonWidget multiplayerButton = new UmbraButtonWidget(startX + BUTTON_WIDTH + SPACING, startY, BUTTON_WIDTH,
		...
		UmbraButtonWidget realmsButton = new UmbraButtonWidget(startX, startY + BUTTON_HEIGHT + SPACING, BUTTON_WIDTH,
		...
		UmbraButtonWidget settingsButton = new UmbraButtonWidget(startX + BUTTON_WIDTH + SPACING,
		...
		UmbraButtonWidget addonsButton = new UmbraButtonWidget(startX, startY + ((BUTTON_HEIGHT + SPACING) * 2),
		...
		UmbraButtonWidget quitButton = new UmbraButtonWidget(startX + BUTTON_WIDTH + SPACING,
		...
		try {
			...
			UmbraButtonWidget modsButton = new UmbraButtonWidget(startX,
		...
		UmbraImageButtonWidget creditsButton = new UmbraImageButtonWidget(width - 20 - 10, height - 20 - 10, 20, 20,
				TextureBank.umbra);
		creditsButton.setPressAction(b -> MC.gui.setScreen(new UmbraCreditsScreen()));
		addRenderableWidget(creditsButton);

		UmbraImageButtonWidget discordButton = new UmbraImageButtonWidget(width - 60, height - 30, 20, 20,
				TextureBank.discord);
	...
	public void extractRenderState(GuiGraphicsExtractor drawContext, int mouseX, int mouseY, float delta) {
		...
		drawContext.text(font, "Umbra " + UmbraClient.UMBRA_VERSION, 2, height - 10, 0xFFFF00FF);

		if (UmbraClient.addons.isEmpty()) {
			...
			int yOffset = 10;
			for (IAddon addon : UmbraClient.addons) {
