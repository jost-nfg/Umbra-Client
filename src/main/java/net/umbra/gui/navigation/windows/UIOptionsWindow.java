...
import net.umbra.gui.components.ExpanderComponent;
import net.umbra.gui.components.ShaderComponent;
...
public class UIOptionsWindow extends Window {
	public UIOptionsWindow() {
		...
		clickGuiButtonComponent.bindProperty(KeybindComponent.SelectedKeyProperty, Umbra.getInstance().guiManager.clickGuiButton, BindingMode.TwoWay);
		clickGuiButtonComponent.setProperty(KeybindComponent.HeaderProperty, Umbra.getInstance().guiManager.clickGuiButton.displayName);
		...
		List<String> fontNames = Umbra.getInstance().fontManager.fonts.keySet().stream().toList();
		...
		fontCombo.setProperty(ComboBoxComponent.ItemsTemplate, name -> {
			...
			label.setProperty(UIElement.FontProperty, Umbra.getInstance().fontManager.getFont((String) name));
			return label;
		});
