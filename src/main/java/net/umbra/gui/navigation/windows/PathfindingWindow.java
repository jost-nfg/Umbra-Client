...
import net.umbra.managers.pathfinding.AbstractPathManager;
import net.umbra.managers.pathfinding.FlyPathManager;
import net.umbra.managers.pathfinding.PathNode;
import net.umbra.managers.pathfinding.TeleportPathManager;
import net.umbra.managers.pathfinding.WalkingPathManager;
...
import net.umbra.settings.types.StringSetting;
import net.umbra.utils.player.MovementUtils;
...
public class PathfindingWindow extends Window implements TickListener, Render3DListener {
	...
	public PathfindingWindow() {
		...
		pathfinderMode = EnumSetting.<Pathfinder>builder().id("goto_pathfinder_mode").displayName("Mode")
				.defaultValue(Pathfinder.Walk).onUpdate(var -> {
					...
					radiusSlider.setProperty(UIElement.IsVisibleProperty, var == Pathfinder.Teleport);
					flyMaxSpeedSlider.setProperty(UIElement.IsVisibleProperty, var == Pathfinder.Fly);
		...
		StringComponent header = new StringComponent("PathFinding");
		...
		StringComponent label = new StringComponent(
				"Automatically walks/flies your player to specific coordinates.");
		stackPanel.addChild(label);

		ComboBoxComponent pathfinderModeComponent = new ComboBoxComponent();
		pathfinderModeComponent.setProperty(ComboBoxComponent.ItemsSourceProperty, List.of(pathfinderMode.getValue().getClass().getEnumConstants()));
		pathfinderModeComponent.bindProperty(ComboBoxComponent.SelectedItemProperty, pathfinderMode, BindingMode.TwoWay);
		stackPanel.addChild(pathfinderModeComponent);
		
		stackPanel.addChild(pathfinderModeComponent);

		CheckboxComponent avoidWaterCheckbox = new CheckboxComponent();
		avoidWaterCheckbox.setProperty(CheckboxComponent.HeaderProperty, avoidWater.displayName);
		avoidWaterCheckbox.bindProperty(CheckboxComponent.IsCheckedProperty, avoidWater, BindingMode.TwoWay);
		stackPanel.addChild(avoidWaterCheckbox);

		CheckboxComponent avoidLavaCheckbox = new CheckboxComponent();
		avoidLavaCheckbox.setProperty(CheckboxComponent.HeaderProperty, avoidLava.displayName);
		avoidLavaCheckbox.bindProperty(CheckboxComponent.IsCheckedProperty, avoidLava, BindingMode.TwoWay);
		stackPanel.addChild(avoidLavaCheckbox);

		radiusSlider = new SliderComponent();
		radiusSlider.setProperty(SliderComponent.HeaderProperty, radius.displayName);
		radiusSlider.setProperty(SliderComponent.MinimumProperty, radius.min_value);
		radiusSlider.setProperty(SliderComponent.MaximumProperty, radius.max_value);
		radiusSlider.bindProperty(SliderComponent.ValueProperty, radius, BindingMode.TwoWay);
		stackPanel.addChild(radiusSlider);

		flyMaxSpeedSlider = new SliderComponent();
		flyMaxSpeedSlider.setProperty(SliderComponent.HeaderProperty, maxSpeed.displayName);
		flyMaxSpeedSlider.setProperty(SliderComponent.MinimumProperty, maxSpeed.min_value);
		flyMaxSpeedSlider.setProperty(SliderComponent.MaximumProperty, maxSpeed.max_value);
		flyMaxSpeedSlider.bindProperty(SliderComponent.ValueProperty, maxSpeed, BindingMode.TwoWay);
		stackPanel.addChild(flyMaxSpeedSlider);

		stackPanel.addChild(new StringComponent("X Coordinate"));
		TextBoxComponent locationXTextBox = new TextBoxComponent();
		locationXTextBox.bindProperty(TextBoxComponent.TextProperty, locationX, BindingMode.TwoWay);
		stackPanel.addChild(locationXTextBox);

		stackPanel.addChild(new StringComponent("Y Coordinate"));
		TextBoxComponent locationYTextBox = new TextBoxComponent();
		locationYTextBox.bindProperty(TextBoxComponent.TextProperty, locationY, BindingMode.TwoWay);
		stackPanel.addChild(locationYTextBox);

		stackPanel.addChild(new StringComponent("Z Coordinate"));
		TextBoxComponent locationZTextBox = new TextBoxComponent();
		locationZTextBox.bindProperty(TextBoxComponent.TextProperty, locationZ, BindingMode.TwoWay);
		...
		GridComponent buttonGrid = new GridComponent();
		buttonGrid.addColumnDefinition(new GridDefinition(1, RelativeUnit.Relative));
		buttonGrid.addColumnDefinition(new GridDefinition(1, RelativeUnit.Relative));
		buttonGrid.setProperty(GridComponent.HorizontalSpacingProperty, 8f);
		
		startButton = new ButtonComponent(startRunnable);
		startButton.setProperty(UIElement.PaddingProperty, new Thickness(8f));
		startButtonText = new StringComponent("Calculate");
		startButton.setContent(startButtonText);
		buttonGrid.addChild(startButton);

		setPositionButton = new ButtonComponent(setPositionRunnable);
		setPositionButton.setProperty(UIElement.PaddingProperty, new Thickness(8f));
		setPositionButton.setContent(new StringComponent("Set Position"));
		buttonGrid.addChild(setPositionButton);

		stackPanel.addChild(buttonGrid);
	...
	public void onRender(Render3DEvent event) {
		if (nodes != null) {
			...
			event.getRenderer().drawBox(startBox, Shader.solid(Colors.Red), 1.0f);
			event.getRenderer().drawBox(endBox, Shader.solid(Colors.Red), 1.0f);
