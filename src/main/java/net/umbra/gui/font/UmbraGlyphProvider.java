...
public class UmbraGlyphProvider implements GlyphProvider {
	@Nullable
	...
	public UmbraGlyphProvider(ByteBuffer fontMemory, FT_Face face, float atlasSize, float emboldenPixels) {
	...
	private UnbakedGlyph loadGlyph(int codepoint, FT_Face f, int glyphIndex) {
		...
		if (width > 0 && height > 0) {
			return new UmbraGlyph(left, top, width, height, advance, glyphIndex);
		}
	...
	private class UmbraGlyph implements UnbakedGlyph {
		final int width, height;
		final float bearingX, bearingY;
		private final GlyphInfo info;
		final int index;

		UmbraGlyph(float left, float top, int width, int height, float advance, int index) {
			this.width = width;
			this.height = height;
