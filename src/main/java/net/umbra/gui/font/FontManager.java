...
public class FontManager {
	private static final String BUILTIN_FONT_PATH = "/assets/umbra/fonts/";
	...
	public void Initialize() {
		...
		if (fonts.get(DEFAULT_FONT) == null) {
			LogUtils.getLogger().error(
					"[Umbra] Default font '{}' failed to load. Drop {}.ttf into src/main/resources{}",
	...
	private void loadBuiltinFonts() {
		for (String id : BUILTIN_FONT_IDS) {
			...
			try (InputStream is = FontManager.class.getResourceAsStream(resourcePath)) {
				if (is == null) {
					LogUtils.getLogger().warn("[Umbra] Built-in font not found: {}", resourcePath);
					continue;
				}
				byte[] bytes = is.readAllBytes();
				fonts.put(id, new UIFont(id, bytes));
				LogUtils.getLogger().info("[Umbra] Loaded built-in font: {}", id);
			} catch (Exception e) {
				LogUtils.getLogger().error("[Umbra] Failed to load built-in font: {}", id, e);
			}
		}
	}

	private void loadUserFonts() {
		File fontDirectory = new File(MC.gameDirectory + File.separator + "umbra" + File.separator + "fonts");
	...
	public static Font loadFontFromBytes(byte[] fontBytes, String id, float emboldenPixels, float atlasSize) throws IOException {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			GlyphProvider provider = new UmbraGlyphProvider(byteBuffer, face, atlasSize, emboldenPixels);
			...
			GlyphStitcher stitcher = new GlyphStitcher(MC.getTextureManager(),
					Identifier.parse("umbra:fonts/" + id.toLowerCase()));
