...
import net.umbra.gui.components.ImageComponent;
import net.umbra.gui.components.ModuleComponent;
import net.umbra.gui.components.TabComponent;
import net.umbra.gui.components.TabItemComponent;
...
import net.umbra.gui.navigation.Page;
import net.umbra.gui.navigation.Popup;
import net.umbra.gui.navigation.Window;
import net.umbra.gui.navigation.huds.ArmorHud;
import net.umbra.gui.navigation.huds.CoordsHud;
import net.umbra.gui.navigation.huds.DayHud;
import net.umbra.gui.navigation.huds.FPSHud;
import net.umbra.gui.navigation.huds.InputHud;
import net.umbra.gui.navigation.huds.ModuleArrayListHud;
import net.umbra.gui.navigation.huds.ModuleSelectorHud;
import net.umbra.gui.navigation.huds.NetherCoordsHud;
import net.umbra.gui.navigation.huds.PingHud;
import net.umbra.gui.navigation.huds.RadarHud;
import net.umbra.gui.navigation.huds.SpeedHud;
import net.umbra.gui.navigation.huds.TimeHud;
import net.umbra.gui.navigation.huds.WatermarkHud;
import net.umbra.gui.navigation.windows.AntiCheatWindow;
import net.umbra.gui.navigation.windows.AuthCrackerWindow;
import net.umbra.gui.navigation.windows.FriendsWindow;
import net.umbra.gui.navigation.windows.PathfindingWindow;
import net.umbra.gui.navigation.windows.SearchWindow;
import net.umbra.gui.navigation.windows.UIOptionsWindow;
import net.umbra.gui.types.GridDefinition;
import net.umbra.gui.types.Rectangle;
import net.umbra.utils.RuntimeEnvironment;
...
import net.umbra.gui.navigation.windows.MacroWindow;
import net.umbra.gui.navigation.windows.SettingsWindow;
import net.umbra.gui.navigation.windows.ToggleHudsTab;
import net.umbra.gui.menu.FlatMenu;
...
import net.umbra.settings.types.FontSetting;
import net.umbra.settings.types.KeybindSetting;
import net.umbra.utils.input.CursorStyle;
import net.umbra.utils.input.Input;
...
public class GuiManager implements KeyDownListener, TickListener, Render2DListener, MouseClickListener,
	...
	public KeybindSetting clickGuiButton = KeybindSetting.builder().id("key.clickgui").displayName("ClickGUI Key")
			.defaultValue(InputConstants.Type.KEYSYM.getOrCreate(GLFW.GLFW_KEY_DELETE)).build();

	public KeybindSetting normalGuiButton = KeybindSetting.builder().id("key.normalgui").displayName("Menu Key")
			.defaultValue(InputConstants.Type.KEYSYM.getOrCreate(GLFW.GLFW_KEY_INSERT)).build();

	private static CursorStyle currentCursor = CursorStyle.Default;
	private static String tooltip = null;
	private final KeyMapping esc = new KeyMapping("key.esc", GLFW.GLFW_KEY_ESCAPE, UmbraClient.UMBRA_CATEGORY);
	...
	// Global HUD Settings
	// Custom title screen conflicts with Lunar Client's own main menu, so it
	// defaults to off when running under Lunar (users can still force-enable it).
	public static BooleanSetting enableCustomTitle = BooleanSetting.builder().id("enable_custom_title")
			.displayName("Enable Custom Title Screen").defaultValue(!RuntimeEnvironment.isLunar()).build();
	...
	public static ShaderSetting foregroundHeaderColor = ShaderSetting.builder().id("ui_header_foreground_color")
			...
			.defaultValue(Shader.solid(new Color(80, 200, 255))).build();
	...
	public static FontSetting fontSetting = FontSetting.builder().id("umbra_font").displayName("Font")
			.description("The font that Umbra will use.").fontName(FontManager.DEFAULT_FONT).build();
	...
	public GuiManager() {
		...
		SettingManager.registerSetting(normalGuiButton);
		...
		// Created last so its listeners run after this manager's: the flat menu
		// renders on top of the ClickGUI/HUD and GuiManager gets first shot at
		// the menu toggle key.
		flatMenu = new FlatMenu();
	}
	...
	public void onKeyDown(KeyDownEvent event) {
		if (clickGuiButton.getValue().getValue() == event.GetKey() && MC.gui.screen() == null && !isKeyboardInputActive) {
			if (flatMenu.isOpen()) {
				// The mouse is already released for the flat menu and the ClickGUI
				// wants it released too, so only swap the menu state.
				flatMenu.setOpen(false);
				setClickGuiOpen(true);
			} else {
		...
		if (normalGuiButton.getValue().getValue() == event.GetKey() && MC.gui.screen() == null && !isKeyboardInputActive) {
			setFlatMenuOpen(!flatMenu.isOpen());
			// Consume the key so the game does not receive the press.
			event.cancel();
		}
	}
	...
	/**
	 * Gets whether or not the flat menu is currently open.
	 *
	 * @return State of the flat menu.
	 */
	public boolean isFlatMenuOpen() {
		return flatMenu.isOpen();
	}

	/**
	 * Opens or closes the flat menu, keeping it mutually exclusive with the
	 * ClickGUI and releasing/grabbing the mouse as needed.
	 */
	public void setFlatMenuOpen(boolean state) {
		if (flatMenu.isOpen() == state)
			return;

		if (state)
			setClickGuiOpen(false);
		flatMenu.setOpen(state);

		if (state) {
			if (MC.mouseHandler.isMouseGrabbed())
				MC.mouseHandler.releaseMouse();
		} else if (!clickGuiOpen && MC.gui.screen() == null && !MC.mouseHandler.isMouseGrabbed()) {
