...
import net.umbra.managers.rotation.goals.Vec3dGoal;
...
import net.umbra.settings.types.RangeSetting;
import net.umbra.utils.types.Range;
import net.umbra.utils.BlockPlacement;
import net.umbra.utils.FindItemResult;
import net.umbra.utils.entity.DamageUtils;
import net.umbra.utils.entity.TargetPriority;
...
public class AutoCrystal extends Module implements TickListener, Render3DListener {
	...
	public void onTick(TickEvent.Pre event) {
		...
		if (rotationGoal != null) {
			Umbra.getInstance().rotationManager.setGoal(Vec3dGoal.builder().goal(rotationGoal)
	...
	private BlockPlacement pickPlaceTarget() {
		...
		for (Player player : Umbra.getInstance().entityManager.getPlayers()) {
			if (player == MC.player || MC.player.distanceToSqr(player) > radius.getValueSqr())
				continue;
			if (!targetFriends.getValue() && Umbra.getInstance().friendsList.contains(player))
				continue;
	...
	private EndCrystal pickAttackTarget() {
		...
		for (Entity entity : Umbra.getInstance().entityManager.getEntities()) {
			...
			for (Player player : MC.level.players()) {
				if (player == MC.player
						|| (!targetFriends.getValue() && Umbra.getInstance().friendsList.contains(player)))
					continue;
