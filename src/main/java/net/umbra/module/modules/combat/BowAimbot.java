...
public class BowAimbot extends Module implements SubtickListener {
	...
	public void onDisable() {
		if (Umbra.getInstance().moduleManager.trajectory.state.getValue())
			Umbra.getInstance().moduleManager.trajectory.toggle();
	...
	public void onSubtick(SubtickEvent event) {
		...
		if (currentTick >= frequency.getValue()) {
			...
			if (temp != null) {
				...
				Umbra.getInstance().rotationManager.setGoal(goal);
			} else
				Umbra.getInstance().rotationManager.setGoal(null);
		}
	}
