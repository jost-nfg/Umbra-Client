...
import net.umbra.settings.types.EntitiesSetting;
...
public class Aimbot extends Module implements SubtickListener, Render3DListener {
	...
	private final BooleanSetting holdToAim = BooleanSetting.builder().id("aimbot_hold_to_aim").displayName("Hold To Aim")
			.description("Only aims while the attack key is held (legit aim-assist style).").defaultValue(false)
			.build();
	...
	public void onSubtick(SubtickEvent event) {
		if (holdToAim.getValue() && !MC.options.keyAttack.isDown()) {
			Umbra.getInstance().rotationManager.setGoal(null);
			currentTarget = null;
			return;
		}
