...
public class AutoEat extends Module implements FoodLevelListener, PlayerHealthListener, ItemUsedListener {
	...
	public void onDisable() {
		Umbra.getInstance().eventManager.RemoveListener(FoodLevelListener.class, this);
	...
	public void onEnable() {
		Umbra.getInstance().eventManager.AddListener(FoodLevelListener.class, this);
			...
			if (isCurrentlyHandEdible()) {
				...
				if (bestFood != null) {
					if (swapBack.getValue()) {
						...
						UmbraClient.LOGGER.info("[Umbra] Setting previous slot to: " + previousSlot);
					}

					lastUsedItemStack = MC.player.getInventory().getItem(foodSlot);
					isEating = true;
					UmbraClient.LOGGER.info("[Umbra] Eating Slot: " + foodSlot);
	...
	public void onItemUsed(ItemUsedEvent.Post event) {
		UmbraClient.LOGGER.info("[Umbra] Item POST");
		if (lastUsedItemStack != null && ItemStack.isSameItem(event.getItemStack(), lastUsedItemStack)) {
			UmbraClient.LOGGER.info("[Umbra] EATING Item was used");
			boolean shouldContinueEating = shouldEat();
			if (!shouldContinueEating) {
				MC.options.keyUse.setDown(false);
				lastUsedItemStack = null;
				UmbraClient.LOGGER.info("[Umbra]No longer eating : " + previousSlot);
				isEating = false;
				if (swapBack.getValue() && previousSlot != -1) {
					UmbraClient.LOGGER.info("[Umbra] Swapping back to : " + previousSlot);
