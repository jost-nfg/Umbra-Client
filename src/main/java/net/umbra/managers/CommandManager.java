...
import net.umbra.command.commands.*;
...
public class CommandManager {
	...
	public static StringSetting PREFIX = StringSetting.builder().id("umbra_prefix").displayName("Prefix")
			.defaultValue(".umbra").build();
		...
		for (Field field : getClass().getDeclaredFields()) {
			if (Command.class.isAssignableFrom(field.getType())) {
				try {
					...
					LogUtils.getLogger().error("Error initializing Umbra commands: " + e.getMessage());
				}
			}
		}
	...
	public void command(String[] commandIn) {
		try {
			...
			if (command == null) {
				sendChatMessage("Invalid Command! Type " + ChatFormatting.AQUA + ".umbra help" + ChatFormatting.RESET
	...
	public static void sendChatMessage(String message) {
		...
		if (mc.gui != null) {
			mc.gui.hud.getChat().addClientSystemMessage(Component.nullToEmpty(ChatFormatting.DARK_AQUA + "[" + ChatFormatting.AQUA + "Umbra"
					+ ChatFormatting.DARK_AQUA + "] " + ChatFormatting.RESET + message));
		}
	}
}
