...
import net.umbra.managers.altmanager.login.MicrosoftAuth;
import net.umbra.mixin.interfaces.IMinecraft;
import net.umbra.utils.system.SystemUtils;
...
/**
 * Class that manages all of the Alt accounts in Umbra.
 */
public class AltManager {
	...
	public void readAlts() {
		try {
			...
			File altFile = new File("umbra_alts.json");
			if (!altFile.exists()) {
	...
	public void saveAlts() {
		try {
			// Finds the file and opens it.
			File altFile = new File("umbra_alts.json");
			LogUtils.getLogger().info("[Umbra] Saving Alts");
			...
			LogUtils.getLogger().error("[Umbra] Failed to save alts");
			exception.printStackTrace();
		}
	}
	...
	public void loginWithAuth(Alt alt, Runnable onComplete) {
		MicrosoftAuth.requestAuthToken((authToken) -> {
			if (authToken != null) {
				...
				LogUtils.getLogger().error("[Umbra] Microsoft authentication failed - no token received.");
			}
			if (onComplete != null) {
				onComplete.run();
			}
