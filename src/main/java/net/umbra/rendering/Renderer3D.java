...
public class Renderer3D extends AbstractRenderer {
	...
	public void render() {
		...
		try {
			...
			try (RenderPass pass = encoder.createRenderPass(() -> "Umbra 3D Shaders", offscreenColorView,
				...
				for (int i = 0; i < batchCount; i++) {
					...
					pass.setUniform("UmbraShaderParams", paramSlices.get(i));
			...
			Umbra.getInstance().compositor.compose(offscreenColorView, offscreenSampler);
		} finally {
			for (DrawBatch batch : pendingBatches)
				batch.mesh.close();
	...
	private void ensureOffscreenResources(int width, int height) {
		...
		destroyOffscreenResources();
		offscreenColorTexture = RenderSystem.getDevice().createTexture("umbra_3d_color",
		...
		offscreenDepthTexture = RenderSystem.getDevice().createTexture("umbra_3d_depth",
	...
	private void ensureVertexGpuBuffer(int requiredSize) {
		if (vertexGpuBuffer == null || vertexGpuBufferSize < requiredSize) {
			...
			vertexGpuBuffer = RenderSystem.getDevice().createBuffer(() -> "umbra_3d_shader", VERTEX_BUFFER_USAGE,
					vertexGpuBufferSize);
		}
	}
