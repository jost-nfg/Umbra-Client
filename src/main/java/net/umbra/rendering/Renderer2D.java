...
import net.umbra.rendering.msaa.IAAHandler;
import net.umbra.rendering.msaa.SSAAHandler;
...
public class Renderer2D extends AbstractRenderer {
	...
	private void executeDraw(List<DrawCmd> draws) {
		...
		if (projectionBuffer == null)
			projectionBuffer = new ProjectionMatrixBuffer("umbra_gui");
		...
		try (RenderPass pass = msaaHandler.beginPass()) {
			...
			for (DrawCmd draw : draws) {
				...
				pass.setUniform("UmbraShaderParams", draw.shaderParamsSlice);
		...
		Umbra.getInstance().compositor.compose(resolved.view(), resolved.sampler());
	}
	...
	private void ensureVertexBuffer(int requiredSize) {
		if (vertexBuffer == null || vertexBuffer.size() < requiredSize) {
			if (vertexBuffer != null)
				vertexBuffer.close();
			vertexBuffer = new MappableRingBuffer(() -> "umbra_gui", VERTEX_BUFFER_USAGE, requiredSize);
		}
	}
	...
	private void ensureGameSnapshot(int width, int height) {
		if (gameSnapshotTexture != null && gameSnapshotWidth == width && gameSnapshotHeight == height)
			return;
		destroyGameSnapshot();
		gameSnapshotTexture = RenderSystem.getDevice().createTexture("umbra_game_snapshot",
