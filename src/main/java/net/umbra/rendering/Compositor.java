...
public final class Compositor implements AutoCloseable {
	...
	private static final RenderPipeline PIPELINE = RenderPipelines.register(
			RenderPipeline.builder(RenderPipelines.GUI_TEXTURED_SNIPPET)
					.withLocation("pipeline/umbra_composite")
	...
	public void compose(GpuTextureView textureView, GpuSampler sampler) {
		...
		if (projectionBuffer == null) {
			projectionBuffer = new ProjectionMatrixBuffer("umbra_composite");
		}
		...
		try {
			...
			if (vertexBuffer == null || vertexBuffer.size() < requiredSize) {
				...
				vertexBuffer = new MappableRingBuffer(() -> "umbra_composite", VERTEX_BUFFER_USAGE, requiredSize);
			}
			...
			try (RenderPass pass = encoder.createRenderPass(() -> "Umbra Composite",
